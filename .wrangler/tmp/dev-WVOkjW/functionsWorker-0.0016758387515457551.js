...
// ../../../.wrangler/tmp/pages-PjlkZ8/functionsWorker-0.0016758387515457551.mjs
...
__name(ctEqual, "ctEqual");
__name2(ctEqual, "ctEqual");
function safeNext(n) {
  n = String(n || "/");
...
async function onRequestPost(context) {
  ...
  function fail(status, code) {
    if (wantsJson) {
      const error = code === "config" ? "Server not configured: set the DEMO_PASSWORD environment variable." : "Incorrect code.";
      return new Response(
  ...
  if (wantsJson) {
    return new Response(
      JSON.stringify({ ok: true, redirect: next }),
...
__name(makeToken2, "makeToken2");
__name2(makeToken2, "makeToken");
...
__name(ctEqual2, "ctEqual2");
__name2(ctEqual2, "ctEqual");
...
async function onRequest2(context) {
  ...
  if (path === "/login" || path === "/login.html" || path.startsWith("/api/") || path === "/favicon.ico" || path === "/robots.txt") {
    return next();
  }
  const token = readCookie(request, "demo_auth");
  if (token && ctEqual2(token, await makeToken2(env))) {
    return next();
  }
...
// .wrangler/tmp/bundle-6tVE5C/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6tVE5C/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.0016758387515457551.js.map
